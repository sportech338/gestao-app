                        ...
                        if submitted:
                            try:
                                with st.spinner(f"Processando pedido #{order_display}..."):
                                    ...
                                    # ✅ SALVA O RASTREIO NA ABA "AGUARDANDO" ANTES DA SHOPIFY
                                    if tracking_number:
                                        salvar_rastreio_na_aba_aguardando(
                                            numero_pedido=order_display,
...
with aba3:
    ...
    df_importacao = dedup(df_importacao, date_col_idx=1)
    ...
    df_importacao = aplicar_filtro_data(df_importacao, date_col_idx=1)
    ...
    df_entregue_ali, df_entregue_est = separar_origem(df_entregue)
    ...
    c1.metric("🟡 Aguardando", contar(df_aguardando))
    c2.metric("🚚 Em trânsito", contar(df_transito))
    c3.metric("⛔ Importação", contar(df_importacao))
    ...
    with t_transito:
        ...
        # =====================================================
        # 🎛️ FILTRO DE ORIGEM (ANÁLISE)
        # =====================================================
        st.markdown("## 📊 Análise de pedidos em trânsito")

        filtro_origem = st.selectbox(
            "Filtrar por origem",
            ["Todas", "🛒 AliExpress", "📦 Estoque"],
            index=0
        )

        df_base = df_transito.copy()

        if filtro_origem != "Todas":
            df_base = df_base[df_base["Origem"] == filtro_origem]

        # =====================================================
        # 🗂️ SUB-ABAS — UMA TABELA POR VEZ
        # =====================================================
        tab_atrasados, tab_risco = st.tabs([
            "🔴 Atrasados",
            "🟡 Em risco"
        ])

        with tab_atrasados:
            df_view = df_base[df_base["Status logístico"] == "🔴 Atrasado"]

            if df_view.empty:
                st.success("✅ Nenhum pedido atrasado.")
            else:
                st.error(f"🚨 {len(df_view)} pedido(s) atrasado(s).")
                st.dataframe(
                    df_view[
        ...
        with tab_risco:
            df_view = df_base[df_base["Status logístico"] == "🟡 Risco"]

            if df_view.empty:
                st.success("✅ Nenhum pedido em risco.")
            else:
                st.warning(f"⚠️ {len(df_view)} pedido(s) em risco.")
                st.dataframe(
                    df_view[
                        ["PEDIDO", "Origem", "RASTREIO", "Dias em trânsito"]
                    ],
...
with aba4:
    ...
    df_kpi["quantity"] = pd.to_numeric(df_kpi.get("quantity", 0), errors="coerce").fillna(0)
    ...
    c2.metric("📦 Itens vendidos", total_itens)
    c3.metric("💰 Faturamento", formatar_moeda(faturamento))
    c4.metric("💸 Ticket médio", formatar_moeda(ticket_medio))
    ...
    # -------------------------------------------------
    # 📥 CARREGAR ABAS DO GOOGLE SHEETS
    # -------------------------------------------------
    @st.cache_data(ttl=300)

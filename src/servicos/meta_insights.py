...
from .sessao_http import get_session, retry_call
from ..recursos.filtros import ensure_act_prefix
from ..regras.metricas import (
    to_float, sum_actions_exact, sum_actions_contains, pick_purchase_totals
)
from ..regras.parsing import dividir_em_janelas, parse_hour_bucket
from ..configuracao import ATTR_KEYS, HOUR_BREAKDOWN
...
def fetch_insights_daily(act_id: str, token: str, api_version: str,
    ...
    chunks = list(dividir_em_janelas(since_str, until_str, max_days=30))
    all_rows: list[dict] = []
...
def fetch_insights_hourly(act_id: str, token: str, api_version: str,
    ...
    act_id = ensure_act_prefix(act_id)
    base_url = f"https://graph.facebook.com/{api_version}/{act_id}/insights"
    fields = ["spend","impressions","clicks","actions","action_values","account_currency","date_start","campaign_id","campaign_name"]
    ...
    dfs = []
    for s_chunk, u_chunk in dividir_em_janelas(since_str, until_str, max_days=30):
        try:
